...
app.put("/:msgId", async (c) => {
  ...
  const msg = db.prepare("SELECT * FROM messages WHERE id = ? AND deleted_at IS NULL").get(msgId);
  if (!msg) return c.json({ error: "message not found or deleted" }, 404);

  const textLatin = stripAccents(parsed.data.text);
  db.prepare(
    "UPDATE messages SET text = ?, text_latin = ?, edited_at = datetime('now') WHERE id = ?"
  ).run(parsed.data.text, textLatin, msgId);
...
app.delete("/:msgId", (c) => {
  ...
  db.prepare("UPDATE messages SET deleted_at = datetime('now') WHERE id = ?").run(msgId);
